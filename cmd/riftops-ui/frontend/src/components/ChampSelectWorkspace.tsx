...
import {
  Ban, Check, CheckCircle2, ChevronDown, Clock3, Eye, Flame, LockKeyhole, Loader2,
...
import {
  ...
  fetchLCUChampSelectSkins, fetchLCURunePages, rerollLCUChampSelect,
...
import type { ChampSelectAction as SelectAction, ChampSelectSession as BaseChampSelectSession } from '../champSelectFlow';
...
export default function ChampSelectWorkspace({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const next = await fetchLCUChampSelect() as Session;
      setSession(next);
  ...
  const activeChampionID = selectedChampion || Number(pending?.championId || championID(localMember || {}));
