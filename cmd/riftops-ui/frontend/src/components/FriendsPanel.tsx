...
function displayName(friend: Friend, streamerMode = false, index = 0): string {
  if (streamerMode) {
    const raw = friend.puuid || friend.id || `${index}`;
...
export default function FriendsPanel({ connected, id }: { connected: boolean; id?: string }) {
  ...
  const { pageVisible, pollInterval, streamerMode } = useLCUConnection();
  ...
  const visibleFriends = useMemo(() => friends
    ...
    .filter((friend) => filter !== 'favorites' || favorites.has(friend.puuid || friend.id || displayName(friend, streamerMode)))
    .filter((friend) => !query.trim() || displayName(friend, streamerMode).toLowerCase().includes(query.trim().toLowerCase()))
    .sort((a, b) => Number(statusTone(b.availability) === 'is-online') - Number(statusTone(a.availability) === 'is-online') || displayName(a, streamerMode).localeCompare(displayName(b, streamerMode))), [filter, friends, query, favorites, streamerMode]);
    ...
    <section id={id} className={`friends-panel ${collapsed ? 'is-collapsed' : ''}`}>
      ...
      {!collapsed && connected && <>
        ...
        {visibleFriends.length > 0 && <div className="friends-panel__list">{visibleFriends.map((friend, index) => {
          const name = displayName(friend, streamerMode, index);
          const friendId = friend.puuid || friend.id || name;
          const favorite = favorites.has(friendId);
          const iconId = friend.profileIconId || friend.iconId;
          return <div className="friends-panel__friend" key={friendId || `${name}-${index}`}>
            ...
            <span className="friends-panel__copy"><strong>{name}</strong><small>{friend.productName || friend.product || statusLabel(friend.availability)}</small></span>
            <span className={`friends-panel__availability ${statusTone(friend.availability)}`}>{statusLabel(friend.availability)}</span>
            <button type="button" className={`friends-panel__favorite ${favorite ? 'is-favorite' : ''}`} onClick={() => toggleFavorite(friend)} aria-label={favorite ? `Remove ${name} from favorites` : `Favorite ${name}`}><Star /></button>
          </div>;
        })}</div>}
      </>}
    </section>
